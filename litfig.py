...
# seeds: per rule, these join the reading set regardless of
# the knee; here, the team's own papers found in the goal query
SEEDS = ["https://openalex.org/W4289978539",   # Di Nucci
         "https://openalex.org/W4401567302",   # Schmid
         "https://openalex.org/W7150767793"]   # Esposito
...
top = cross[:knee + 1]
top += [w for w in cross
        if w["id"] in SEEDS and w not in top]  # seeds bypass knee
...
def rowlabel(i, w):
    return ("s" if w["id"] in SEEDS else str(i + 1))


with open("fig/littab.tex", "w") as f:
    f.write("% generated by litfig.py; do not hand-edit\n")
    f.write("\\begin{tabular}"
            "{@{}rp{0.56\\linewidth}p{0.15\\linewidth}rrccc@{}}\n"
            "\\toprule\n\\# & paper & venue & year & cites & "
            "trust & productivity & human studies\\\\\n"
            "\\midrule\n")
    for i, w in enumerate(top):
        ttl = esc(w["title"] or "?")
        ttl = ttl[:78] + ("..." if len(ttl) > 78 else "")
        f.write("%s & %s (%s) & %s & %d & %d & %s & %s & %s\\\\\n" %
                (rowlabel(i, w), ttl, esc(first_author(w)),
                 esc(venue(w))[:26],
