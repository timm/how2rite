...
Ask: are the construct poles the distinctions you would draw?
Are any ratings unfair to a colleague?
Should a person (or a construct) be added or dropped?
Is five top-cited papers per author the right evidence base?\\[0.2em]
To change the outcome: edit the {\ttfamily CONSTRUCTS} table (or
{\ttfamily ELEMENTS}) in {\ttfamily focus.py}, then {\ttfamily make
focus}; the trees, the pairings of \S\ref{sec:map}, and the topic
arithmetic all recompute from that one table.
\end{pause}
